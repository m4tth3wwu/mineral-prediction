"""P1-I: fixed lr.001 pair against frozen lr.003; train96 unchanged."""
...
REFERENCE=ROOT/'p1h_quadrature/20260922T072637Z_p1h_train96'
...
def session():return read(MEM/'P1I_SESSION.json')
def frozen_check():
...
def prepare(out):
    ...
    p=read(OLD/'PROTOCOL.json');p.update(stage='P1-I',version=1,frozen_at=utc(),base_commit=session()['base_commit'],run_id=out.name,task_path=session()['prompt_path'],task_sha256=sha(REPO/session()['prompt_path']),objective='256*logsumexp(log(w96)+g96)-sum(g(archived events data23)) + lambda*original penalty',training_grid=96,diagnostic_grid=48,monitor_grids=[48,96,192],evaluation_grids=[384,768],comparison='Only learning rate changes from P1-H .003 to .001; train96, independent monitor192 and fixed step2000 budget unchanged. All4 endpoints evaluated on384/768.',endpoint_policy='No adaptive refinement/retraining if384/768 fidelity fails; mark unresolved. NaN aborts further training. No best checkpoint selection.',scope='Exactly2 new trajectories of2000 updates. Only lr .003 to .001; no other optimizer/model/grid/seed changes. Exactly one candidate full displacement on each new endpoint temporary copy; not a2001st training update.')
    p['optimizer']['lr']=.001
    p['reference']=rel(REFERENCE)
    p['interpretation']='Fixed-budget learning-rate intervention. Lower loss oscillation or better teacher recovery does not imply stationarity or better optimum; smaller lr can slow fitting. No additional steps/lr sweep/checkpoint selection. Compare matched 2000-update trajectories, and their complete fixed-step curves.'
    ...
    p['linear_diagnostic']=dict(prediction='gradient(theta[t-1]) dot (theta[t]-theta[t-1])',reversal='prediction < -1e-8 AND actual total delta >1e-8',scope='Both P1-H and P1-I record actual update predictions on the same96 objective: compare reversals, positive predictions (>1e-8), and actual increases (>1e-8); no dense-grid per-step derivative claim.')
    ...
    sr,sg=streaming_ppp_value_and_grad(m,events,z['xy'],z['w'],1.,512,wrt=tuple(m.parameters()));checks['streaming_gradient']=torch.allclose(g0,torch.cat([a.flatten() for a in sg]),atol=1e-10,rtol=1e-10);checks['streaming_value']=abs(v-float(sr.objective))<=1e-9
    ...
    reference=f.tensor_load(REFERENCE/'raw/INPUTS.pt');checks['P1H_inputs']=torch.equal(events,reference['events']) and e.state_hash(reference['teacher'])==e.state_hash(teacher) and all(e.state_hash(x)==e.state_hash(state) for x in reference['starts'].values())
    checks['P1H_initial_objective']=abs(v-read(REFERENCE/'INITIAL_DIAGNOSTICS.json')['empirical96'])<=1e-10
    checks['P1H_verified']=read(REFERENCE/'FINAL_VERIFY.json')['passed'] and read(REFERENCE/'EVALUATION_VERIFY.json')['passed']
...
def trajectory(out,label,state,lam,events,grids,delta):
    m=e.model(state);opt=torch.optim.Adam(m.parameters(),lr=.001,betas=(.9,.999),eps=1e-8,weight_decay=0,amsgrad=False);z=grids[96];xy,w,t=z['xy'],z['w'],z['target'];history=[];obs=[];prev=e.vec(m);recent=deque(maxlen=3);save_until=-1;start=time.perf_counter();previous_gradient=None
