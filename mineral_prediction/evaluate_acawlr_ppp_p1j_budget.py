"""P1-J fixed endpoint evaluation; Adam.step is forbidden in this module."""
...
def diagnostics(out,label,cp,events,checks):
    ...
    pg=cp['optimizer']['param_groups'][0];ids=pg['params'];states=cp['optimizer']['state'];checks[label+'_optimizer_recipe']=pg['lr']==.001 and tuple(pg['betas'])==(.9,.999) and pg['eps']==1e-8 and pg['weight_decay']==0 and not pg['amsgrad'] and all(int(states[i]['step'])==6000 for i in ids)
    moment=torch.cat([states[i]['exp_avg'].flatten() for i in ids]);variance=torch.cat([states[i]['exp_avg_sq'].flatten() for i in ids]);gr=vectors[96]['total'];b1,b2=pg['betas'];lr,eps=pg['lr'],pg['eps'];t=6001
    ...
    torch.save(dict(candidate=d,moment_old=moment,variance_old=variance,moment_candidate=mn,variance_candidate=vn,model_hash=before,candidate_step=6001,optimizer_steps=0),out/'raw'/f'{label}_candidate.pt')
...
def run(out):
    torch.set_num_threads(1);assert h.read(out/'TRAINING_END.json')['updates']==8000
    ...
    with patch.object(torch.optim.Adam,'step',side_effect=AssertionError('No evaluation updates')),patch.object(torch.optim.SGD,'step',side_effect=AssertionError('No evaluation updates')):
        for stage,folder in [(2000,h.REFERENCE),(4000,out),(6000,out)]:
            for label,lam in zip(h.NAMES,[0.,1.]):
                key=f'step{stage}_{label}';path=folder/'raw/states'/f'{label}_{stage:04d}.pt';checkpoint_hashes[f.rel(path)]=f.sha(path);cp=f.tensor_load(path);m=e.model(cp['model']);before=e.state_hash(m.state_dict());checks[key+f'_step{stage}']=cp['step']==stage and all(int(v['step'])==stage for v in cp['optimizer']['state'].values());metrics={};independent_checks={}
                for n in [384,768]:
                    ...
                    if stage==2000:
                        old=h.read(h.REFERENCE/'ENDPOINTS.json')['lr0.001_'+label]['metrics'][str(n)];checks[key+f'_archive{n}']=all(abs(met[k]-old[k])<=1e-12 for k in ['KL','TV','logZ','q_ratio','field_ratio','b_RMSE'])
                    print(key,n,json.dumps({k:met[k] for k in ['KL','q_ratio','field_ratio','b_RMSE']}),flush=True)
                checks[key+'_unchanged']=e.state_hash(m.state_dict())==before;results[key]=dict(training_grid=96,learning_rate=.001,budget=stage,lambda_multiplier=lam,model_hash=before,metrics=metrics,fidelity=e.fidelity(metrics['384'],metrics['768']),independent=independent_checks);h.write(out/'ENDPOINTS.json',results)
                if stage==6000:
