...
'''
    ...
    for i,(key,component) in enumerate([('g',None),('b',0),('b',1)]):
        ...
        for j,d in enumerate(data):
            edge=np.linspace(-1,1,97);edge=edge+.12*np.sin(np.pi*edge);im=axes[i,j].pcolormesh(edge*60,edge*40,d.numpy().reshape(96,96).T,cmap='coolwarm',vmin=-bound,vmax=bound,shading='flat');axes[i,j].set_aspect('equal')
    ...
    fig.suptitle('Fixed states, 96²; same color scale within each field; nonuniform spatial cells');fig.savefig(out/'figures/fixed_fields.png',dpi=140);plt.close(fig)
