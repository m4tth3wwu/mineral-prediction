"""P1-I fixed endpoint evaluation; Adam.step is forbidden in this module."""
...
def diagnostics(out,label,cp,events,checks):
    ...
    pg=cp['optimizer']['param_groups'][0];ids=pg['params'];states=cp['optimizer']['state'];checks[label+'_optimizer_recipe']=pg['lr']==.001 and tuple(pg['betas'])==(.9,.999) and pg['eps']==1e-8 and pg['weight_decay']==0 and not pg['amsgrad'] and all(int(states[i]['step'])==2000 for i in ids)
...
def run(out):
    ...
    with patch.object(torch.optim.Adam,'step',side_effect=AssertionError('No evaluation updates')),patch.object(torch.optim.SGD,'step',side_effect=AssertionError('No evaluation updates')):
        for train_n,folder in [(.003,h.REFERENCE),(.001,out)]:
            for label,lam in zip(h.NAMES,[0.,1.]):
                key=f'lr{train_n:.3f}_{label}';path=folder/'raw/states'/f'{label}_2000.pt';checkpoint_hashes[f.rel(path)]=f.sha(path);cp=f.tensor_load(path);m=e.model(cp['model']);before=e.state_hash(m.state_dict());checks[key+'_step2000']=cp['step']==2000 and all(int(v['step'])==2000 for v in cp['optimizer']['state'].values());metrics={};independent_checks={}
                for n in [384,768]:
                    ...
                    if train_n==.003:
                        old=h.read(h.REFERENCE/'ENDPOINTS.json')['train96_'+label]['metrics'][str(n)];checks[key+f'_archive{n}']=all(abs(met[k]-old[k])<=1e-12 for k in ['KL','TV','logZ','q_ratio','field_ratio','b_RMSE'])
                    print(key,n,json.dumps({k:met[k] for k in ['KL','q_ratio','field_ratio','b_RMSE']}),flush=True)
                checks[key+'_unchanged']=e.state_hash(m.state_dict())==before;results[key]=dict(training_grid=96,learning_rate=train_n,lambda_multiplier=lam,model_hash=before,metrics=metrics,fidelity=e.fidelity(metrics['384'],metrics['768']),independent=independent_checks);h.write(out/'ENDPOINTS.json',results)
                if train_n==.001:
